...
function refocusConnectSocket(app, token) {
  ...
  const botActionAddEventName = 'refocus.internal.realtime.bot.action.add';
  const botActionChangedEventName = 'refocus.internal.realtime.bot.action.update';
  ...
  socket.on(botActionAddEventName, function(data) {
     const eventData = JSON.parse(data);
     const room = eventData[botActionAddEventName];
     app.emit('refocus.bot.actions', room);
  });

  socket.on(botActionChangedEventName, function(data) {
     const eventData = JSON.parse(data);
     const room = eventData[botActionChangedEventName];
     app.emit('refocus.bot.actions', room);
  });
